...
fn replay_user_prompt_submit_binds_provider_session() {
    ...
    let events = adapter.normalize_event("claude-code", &payload).unwrap();
...
#[test]
fn replay_stop_failure_maps_to_error() {
    let adapter = ClaudeAdapter;
    let payload = json!({
        "code_bar_session_id": "session-1",
...
#[test]
fn replay_approval_requested_maps_to_approval_event() {
    let adapter = ClaudeAdapter;
    let payload = json!({
        "code_bar_session_id": "session-1",
        "hook_event_name": "ApprovalRequested",
        "title": "Need approval",
...
fn start_spec_injects_bootstrap_and_bridge() {
    let adapter = ClaudeAdapter;
    let session = sample_session(ProviderKind::Claude, None);
    let worktree = sample_worktree("/tmp/worktree-claude");
    ...
    assert_eq!(spec.cwd, "/tmp/worktree-claude");
...
fn resume_spec_adds_provider_resume_args() {
    let adapter = ClaudeAdapter;
    let session = sample_session(ProviderKind::Claude, Some("claude-provider-1"));
    ...
    assert_eq!(
        spec.args,
        vec!["resume".to_string(), "claude-provider-1".to_string()]
    );
