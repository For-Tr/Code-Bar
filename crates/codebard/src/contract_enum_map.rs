...
pub fn map_session_launch_mode_to_contract(
    mode: domain::SessionLaunchMode,
) -> contract_domain::SessionLaunchMode {
    match mode {
...
pub fn map_session_state_to_contract(state: domain::SessionState) -> contract_domain::SessionState {
    match state {
        ...
        domain::SessionState::PreparingWorkspace => {
            contract_domain::SessionState::PreparingWorkspace
        }
...
pub fn map_worktree_source_to_contract(
    source: domain::WorktreeSource,
) -> contract_domain::WorktreeSource {
    match source {
...
pub fn map_worktree_lifecycle_state_to_contract(
    state: domain::WorktreeLifecycleState,
) -> contract_domain::WorktreeLifecycleState {
    match state {
        domain::WorktreeLifecycleState::Preparing => {
            contract_domain::WorktreeLifecycleState::Preparing
        }
        ...
        domain::WorktreeLifecycleState::CleanupPending => {
            contract_domain::WorktreeLifecycleState::CleanupPending
        }
...
pub fn map_worktree_cleanup_policy_to_contract(
    policy: domain::WorktreeCleanupPolicy,
) -> contract_domain::WorktreeCleanupPolicy {
    match policy {
        domain::WorktreeCleanupPolicy::Manual => contract_domain::WorktreeCleanupPolicy::Manual,
        domain::WorktreeCleanupPolicy::AutoOnTaskDone => {
            contract_domain::WorktreeCleanupPolicy::AutoOnTaskDone
        }
...
pub fn map_plan_step_status_to_contract(
    status: domain::PlanStepStatus,
) -> contract_domain::PlanStepStatus {
    match status {
