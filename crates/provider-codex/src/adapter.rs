...
fn ensure_provider(provider: ProviderKind) -> DomainResult<()> {
    if provider != ProviderKind::Codex {
        return Err(ErrorEnvelope::new(
            ...
            format!("CodexAdapter cannot handle provider {provider:?}"),
            false,
        ));
    }
    Ok(())
...
fn build_spec(
    ...
    let args = if resume {
        ...
        vec!["exec".to_string(), "--color".to_string(), "never".to_string()]
    };

    let mut env = HashMap::new();
    env.insert("CODEBAR_PROVIDER".to_string(), "codex".to_string());
