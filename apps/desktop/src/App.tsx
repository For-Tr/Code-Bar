...
import type { BackfilledSessionBinding } from "@codebar/contracts";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const u5b = listen<{ sessionId: string; groups: import("./store/scmStore").ScmStatusGroups }>(
      "scm-status-update",
      ({ payload }) => {
        setScmStatus(payload.sessionId, payload.groups);
      }
    );
    ...
    const u7 = listen<{ sessionId: string; runnerType: string; providerSessionId: string }>(
      "provider-session-bound",
      ({ payload }) => {
        if (!payload.sessionId || !payload.providerSessionId) return;
        const session = useSessionStore
          .getState()
          .sessions.find((x) => x.id === payload.sessionId);
        const existing = session?.providerSessionId?.trim();
        // 避免被“新建但空壳”的 provider 会话覆盖已有可恢复会话 ID
        if (existing && existing !== payload.providerSessionId) {
          return;
        }
