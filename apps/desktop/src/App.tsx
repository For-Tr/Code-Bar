...
import { bindProviderSession, ensureDaemonReady, getDaemonDiagnostics, getDaemonNextAction, listDaemonApprovals, listDaemonSessions, mapDaemonSessionToUiSession, syncWorkspaceToDaemon } from "./services/daemonCommands";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    return () => {
      [u1, u2, u3, u4, u5, u5b, u5c, u5d, u6, u7, u8].forEach((p) => p.then((f) => f()).catch(() => {}));
    };
  }, [appendOutput, updateSession, setDiffFiles, setScmSnapshot, setScmStatus, setScmDiffOverride, refreshSessionDiff, setApprovals, setNextAction, setDiagnostics, diagnosticsBySessionId]);
