...
use codebar_contracts::rpc::{
    BackfillSessionBindingInput, BackfilledSessionBinding, DeletedSessionRef, DeletedUiState,
    DeletedWorkspaceRef, SaveRecoveryBindingInput,
};
...
impl DeletedSessionRefExt for DeletedSessionRef {
    fn normalized(self) -> Option<Self> {
        let session_id = self.session_id.trim().to_string();
...
impl DeletedWorkspaceRefExt for DeletedWorkspaceRef {
    fn normalized(self) -> Option<Self> {
...
fn unique_session_refs(values: Vec<DeletedSessionRef>) -> Vec<DeletedSessionRef> {
    ...
    for value in values.into_iter().filter_map(|value| value.normalized()) {
        let key = (
